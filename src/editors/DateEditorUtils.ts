...
const monthUpdate = (arrName: 'monthNames' | 'monthNamesShort' | 'dayNames' | 'dayNamesShort') => (
  v: string,
  i18n: I18nSettings
): number | null => {
...
const dayNames: Days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
...
const setGlobalDateI18n = (i18n: I18nSettingsOptional): I18nSettings => (globalI18n = assign(globalI18n, i18n));
...
const formatFlags: Record<string, (dateObj: Date, i18n: I18nSettings) => string> = {
  D: (dateObj: Date): string => String(dateObj.getDate()),
  DD: (dateObj: Date): string => pad(dateObj.getDate()),
  Do: (dateObj: Date, i18n: I18nSettings): string => i18n.DoFn(dateObj.getDate()),
  d: (dateObj: Date): string => String(dateObj.getDay()),
  dd: (dateObj: Date): string => pad(dateObj.getDay()),
  ddd: (dateObj: Date, i18n: I18nSettings): string => i18n.dayNamesShort[dateObj.getDay()],
  dddd: (dateObj: Date, i18n: I18nSettings): string => i18n.dayNames[dateObj.getDay()],
  ...
  S: (dateObj: Date): string => String(Math.round(dateObj.getMilliseconds() / 100)),
  SS: (dateObj: Date): string => pad(Math.round(dateObj.getMilliseconds() / 10), 2),
  SSS: (dateObj: Date): string => pad(dateObj.getMilliseconds(), 3),
  a: (dateObj: Date, i18n: I18nSettings): string => (dateObj.getHours() < 12 ? i18n.amPm[0] : i18n.amPm[1]),
  A: (dateObj: Date, i18n: I18nSettings): string =>
    dateObj.getHours() < 12 ? i18n.amPm[0].toUpperCase() : i18n.amPm[1].toUpperCase(),
...
const setGlobalDateMasks = (masks: { [key: string]: string }): { [key: string]: string } => assign(globalMasks, masks);
...
const format = (dateObj: Date | number, mask: string = globalMasks['default'], i18n: I18nSettingsOptional = {}): string => {
  ...
  const combinedI18nSettings: I18nSettings = assign(assign({}, globalI18n), i18n);
...
function parse(dateStr: string, format: string, i18n: I18nSettingsOptional = {}): Date | null {
  ...
  for (let i = 1; i < matches.length; i++) {
    ...
    const value = parser ? parser(matches[i], combinedI18nSettings) : +matches[i];
